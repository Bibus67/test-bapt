            ...
            const i18n = {
                fr: {
                    // dynamique (script)
                    noColors: 'Aucune couleur détectée',
                    importPrompt: 'Importez un SVG pour détecter les couleurs',
                    loading: 'Chargement...',
                    gradients: 'DÉGRADÉS',
                    gradient: 'Dégradé',
                    convertToGradient: 'Convertir en dégradé',
                    removeShade: 'Supprimer cette nuance',
                    addShade: 'Ajouter une nuance',
                    smoothGradient: 'Lisser ce dégradé',
                    convertToSolid: 'Convertir en couleur unie',
                    noSvgLoaded: 'Aucun SVG chargé',
                    editActive: '✏️ Quitter l\'édition',
                    editInactive: '✏️ Édition',
                    // statique (data-i18n)
                    pageTitle: 'Éditeur de couleurs SVG',
                    undo: 'Annuler (Ctrl+Z)',
                    redo: 'Rétablir (Ctrl+Y)',
                    editMode: '✏️ Édition',
                    importSvg: 'Importer SVG',
                    exportBtn: 'Exporter ↗',
                    detectedColors: 'Couleurs détectées',
                    resetColors: 'Réinitialiser les couleurs',
                    rotationTitle: 'Rotation de l\'image',
                    resetRotation: 'Réinitialiser la rotation',
                    mirrorTitle: 'Miroir',
                    resetMirrors: 'Réinitialiser les miroirs',
                    mirrorH: 'Miroir horizontal',
                    mirrorV: 'Miroir vertical',
                    noSvgState: 'Aucun fichier SVG chargé',
                    clickToImport: 'Cliquez ici ou sur "Importer SVG" pour commencer',
                ...
                en: {
                    ...
                    importPrompt: 'Import an SVG to detect colors',
                    loading: 'Loading...',
                    gradients: 'GRADIENTS',
                    gradient: 'Gradient',
                    convertToGradient: 'Convert to gradient',
                    removeShade: 'Delete this shade',
                    addShade: 'Add a shade',
                    smoothGradient: 'Smooth this gradient',
                    convertToSolid: 'Convert to solid color',
                    noSvgLoaded: 'No SVG loaded',
                    editActive: '✏️ Exit edit',
                    editInactive: '✏️ Edit',
                    pageTitle: 'SVG Color Editor',
                    undo: 'Undo (Ctrl+Z)',
                    redo: 'Redo (Ctrl+Y)',
                    editMode: '✏️ Edit',
                    ...
                    rotationTitle: 'Image rotation',
                    resetRotation: 'Reset rotation',
                    mirrorTitle: 'Mirror',
                    resetMirrors: 'Reset mirrors',
                    mirrorH: 'Horizontal mirror',
                    mirrorV: 'Vertical mirror',
                    noSvgState: 'No SVG file loaded',
                    clickToImport: 'Click here or "Import SVG" to get started',
                    exportTitle: 'Export',
                    formatLabel: 'Format',
            ...
            function applyLang() {
                document.querySelectorAll('.sve-lang-opt').forEach(opt => {
                    opt.classList.toggle('sve-active', opt.dataset.lang === currentLang);
                });
                ...
                if (editBtn) editBtn.textContent = editModeActive ? t('editActive') : t('editInactive');
                if (mainColorGroups.length > 0 || gradientGroups.length > 0) _refreshPalette();
                else {
                    const noCol = document.querySelector('.sve-no-colors');
                    if (noCol) noCol.textContent = t(currentSVG ? 'noColors' : 'importPrompt');
                }
            }
            ...
            if (langToggle) {
                langToggle.addEventListener('click', function(e) {
                    const opt = e.target.closest('.sve-lang-opt');
                    if (!opt || opt.dataset.lang === currentLang) return;
                    currentLang = opt.dataset.lang;
                    applyLang();
                });
            }
            applyLang(); // init état FR actif
            ...
            function resetColors() {
                ...
                colorPalette.innerHTML = `<div class="sve-no-colors">${t('loading')}</div>`;
                    ...
                    gradients.forEach(gradient => {
                        ...
                        if (stopGroups.length > 0) {
                            ...
                            gradientGroups.push({
                                ...
                                label: 'Dégradé ' + gradientCounter, gradientNum: gradientCounter,
            ...
            // Rafraîchit la palette sans re-détecter les couleurs (préserve stopGroups et scroll)
            function _refreshPalette() {
                const cont = document.querySelector('.sve-gradients-compact-container');
                const scrollTop = cont ? cont.scrollTop : 0;
                displayColors(mainColorGroups, gradientGroups);
                requestAnimationFrame(() => {
                    const newCont = document.querySelector('.sve-gradients-compact-container');
                    if (newCont) newCont.scrollTop = scrollTop;
                });
            }

            function displayColors(mainColorGroups, gradientGroups) {
                const _cont = document.querySelector('.sve-gradients-compact-container');
                const _scrollTop = _cont ? _cont.scrollTop : 0;

                if (mainColorGroups.length === 0 && gradientGroups.length === 0) {
                    colorPalette.innerHTML = `<div class="sve-no-colors">${t('noColors')}</div>`;
                    return;
                }
                ...
                mainColorGroups.forEach((group, groupIndex) => {
                    ...
                    toGradBtn.innerHTML = '◑';
                    Tooltip.attach(toGradBtn, () => t('convertToGradient'));
                ...
                if (isCompact) {
                    ...
                    title.textContent = `${t('gradients')} (${gradientGroups.length})`;
                    ...
                    gradientGroups.forEach((gradientEntry, gradientIndex) => {
                        ...
                        orderedGroups.forEach((stopGroup, displayIdx) => {
                            ...
                            // Bouton ✕ supprimer cette nuance
                            const delBtn = document.createElement('button');
                            delBtn.className = 'sve-gradient-delete-btn';
                            delBtn.innerHTML = '×';
                            Tooltip.attach(delBtn, () => t('removeShade'));
                        ...
                        addBtn.innerHTML = '+';
                        Tooltip.attach(addBtn, () => t('addShade'));
                        ...
                        smoothBtn.innerHTML = '✦';
                        Tooltip.attach(smoothBtn, () => t('smoothGradient'));
                        ...
                        Tooltip.attach(removeGradBtn, () => t('convertToSolid'));
                        removeGradBtn.addEventListener('click', (e) => { e.stopPropagation(); Tooltip.hide(); convertGradientToMainColor(gradientIndex); });

                        const btnGroup = document.createElement('div');
                        btnGroup.className = 'sve-gradient-compact-btns';
                        btnGroup.appendChild(addBtn);
                        btnGroup.appendChild(removeGradBtn);
                        btnGroup.appendChild(smoothBtn);
                    ...
                    gradientGroups.forEach((gradientEntry, gradientIndex) => {
                        ...
                        gradientLabel.textContent = (t('gradient') + ' ' + (gradientEntry.gradientNum || gradientEntry.label.replace(/\D+/g, ''))).toUpperCase();
                        ...
                        Tooltip.attach(removeGradBtnN, () => t('convertToSolid'));
                        ...
                        orderedGroupsN.forEach((stopGroup, displayIdx) => {
                            ...
                            deleteBtn.innerHTML = '×';
                            Tooltip.attach(deleteBtn, () => t('removeShade'));
                        ...
                        addBtnN.innerHTML = '+';
                        Tooltip.attach(addBtnN, () => t('addShade'));
                ...
                // Restaurer la position de scroll de la palette
                requestAnimationFrame(() => {
                    const newCont = document.querySelector('.sve-gradients-compact-container');
                    if (newCont && _scrollTop > 0) newCont.scrollTop = _scrollTop;
                });
            }
            ...
            function updateGradientColorGroup(gradientIndex, stopGroupIndex, newColor) {
                ...
                // Si le dégradé est lissé, propager la nouvelle couleur dans _originalStops
                if (gradientEntry.smoothed && gradientEntry._originalStops && gradientEntry._originalStops[stopGroupIndex]) {
                    gradientEntry._originalStops[stopGroupIndex].color = newColor;
                }
            ...
            function smoothSingleGradient(gradientIndex) {
                ...
                // Sauvegarder depuis stopGroups (pas les stops DOM bruts qui peuvent avoir des
                // intermédiaires sans stop-color → noirs), mais avec les vrais offsets du 1er stop de chaque groupe
                const n0 = gradientEntry.stopGroups.length;
                gradientEntry._originalStops = gradientEntry.stopGroups.map((sg, i) => {
                    const domOffset = sg.stops[0] ? (sg.stops[0].getAttribute('offset') || null) : null;
                    const offset = domOffset !== null ? domOffset : (n0 === 1 ? '0%' : (i / (n0 - 1) * 100).toFixed(2) + '%');
                    return { offset, color: sg.color, opacity: null };
                });
                ...
                // Mettre à jour stopGroups.stops pour pointer sur les nouveaux nodes DOM
                const newDomStops = Array.from(gradient.querySelectorAll('stop'));
                gradientEntry.stopGroups.forEach((sg, i) => {
                    sg.stops = newDomStops[i] ? [newDomStops[i]] : [];
                });
            ...
            function unsmoothedSingleGradient(gradientIndex) {
                ...
                if (gradientEntry._originalStops && gradientEntry._originalStops.length > 0) {
                    ...
                    // Ne pas appeler redistributeStopOffsets : les offsets originaux sont déjà corrects
                } else {
                ...
                // Reconstruire stopGroups depuis les stops DOM restaurés (sans detectColors qui fusionne)
                const restoredStops = Array.from(gradient.querySelectorAll('stop'));
                gradientEntry.stopGroups = restoredStops.map(stop => ({
            ...
            // Retourne le padding à ajouter au viewBox pour ne pas couper strokes/filtres
            function _getExportPad(svgEl) {
                let maxHalfStroke = 0;
                try {
                    // Lire stroke-width sur tous les éléments (attribut direct + style calculé)
                    svgEl.querySelectorAll('*').forEach(el => {
                        ...
                        const swComp = parseFloat(getComputedStyle(el).strokeWidth) || 0;
                        const sw = isNaN(swAttr) ? swComp : swAttr;
                        if (sw > 0) maxHalfStroke = Math.max(maxHalfStroke, sw / 2);
                    });
                } catch(e) {}
                // +4 unités de marge : anti-aliasing, sub-pixel rendering, arrondis
                return Math.ceil(maxHalfStroke) + 4;
            }
            ...
            function exportSVG(filename = null) {
                ...
                // Nettoyer le style CSS du canvas (zoom/pan ne doit pas être dans le fichier)
                svgClone.style.transform = '';
                svgClone.style.transformOrigin = '';
                svgClone.style.margin = '';

                const pad = _getExportPad(currentSVG);

                // Sans transformation : recalculer le viewBox sur le contenu réel + padding stroke
                if (currentRotation === 0 && !mirrorHorizontal && !mirrorVertical) {
                    try {
                        ...
                        if (bbox.width > 0 && bbox.height > 0) {
                            ...
                            svgClone.setAttribute('width', bbox.width + pad * 2);
                            svgClone.setAttribute('height', bbox.height + pad * 2);
                        }
                    } catch(e) {}
                ...
                if (currentRotation !== 0 || mirrorHorizontal || mirrorVertical) {
                    ...
                    // Ajuster le viewBox pour inclure toute l'image transformée + padding stroke
                    ...
                    const newX = cx - newWidth / 2 - pad;
                    const newY = cy - newHeight / 2 - pad;

                    svgClone.setAttribute('viewBox', `${newX} ${newY} ${newWidth + pad * 2} ${newHeight + pad * 2}`);
                    svgClone.setAttribute('width', newWidth + pad * 2);
                    svgClone.setAttribute('height', newHeight + pad * 2);
                }
            ...
            function exportRaster(format, exportScale = 2, filename = null) {
                ...
                try {
                    ...
                    // Padding stroke (demi stroke-width + 1px AA) converti en pixels canvas
                    const strokePad = _getExportPad(currentSVG);
                    // JPEG : padding supplémentaire 5% pour fond blanc propre
                    const jpegExtra = format === 'jpeg' ? Math.ceil(Math.max(newWidth, newHeight) * 0.05) : 0;
                    const pad = strokePad + jpegExtra;
                    ...
                    // Ajuster le viewBox du SVG cloné en incluant le padding stroke
                    const vbX = cx - newWidth / 2 - pad;
                    const vbY = cy - newHeight / 2 - pad;
                    const vbW = newWidth  + pad * 2;
                    const vbH = newHeight + pad * 2;
                    ...
                    img.onload = function () {
                        try {
                            // Le padding est déjà dans le viewBox du SVG — dessiner à (0,0) taille canvas complète
            ...
            function _setEditUIActive(active) {
                ...
                btn.innerHTML = active ? t('editActive') : t('editInactive');
                _syncUndoRedoBtns();
            }
